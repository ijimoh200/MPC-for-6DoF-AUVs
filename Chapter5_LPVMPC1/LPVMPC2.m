function [ui, xi, yi, e_l, e_a, dXpred] = LPVMPC2(eta_ini,nv_ini,yref,nu_c,noise,Ts,Tf)
...
xmin = -[inf; inf; inf; inf; 2*pi/5; inf; 1.5; 1; 0.5; inf; inf; inf];
...
tau_wave = 0;
 ...
 for k = 1:NoS 
    ...
    % Update AUV model
    [~,M, Jk, Ck, Dk, gk] = Naminow_AUV(x_,tau_,0*nu_c,tau_wave);
    ...
    x(:,k+1) = solver_RK(x(:,k),Ts,tau(:,k),nu_c(:,k),tau_wave)+noise(:,k);
    dx(:,k+1) = x(:,k+1) - x(:,k);
    y_ = y(:,k);
    x_ = x(:,k);  
    yref_ = yref(:,k);
