function [xidot,M, Jk, Ck, Dk, gk] = Naminow_AUV(xi,ui,nu_c,dw)
% [xidot,M, Jk, Ck, Dk, gk] = Naminow_AUV(xi,ui,nu_c,dw) returns the 
...
u = xi(7)-nu_c(1);              v =xi(8)-nu_c(2);                   w = xi(9)-nu_c(3);   
...
%  M = blkdiag(m,m,m,Ixx,Iyy,Izz)+blkdiag(Xud,Yvd,Zwd,Kpd,Mqd,Nrd);

% nnn = inv(M)

% Coriolis and centripetal matrix
CRB = [zeros(3) -m*SS(v1)-m*SS(v2)*SS(rgb); -m*SS(v1)+m*SS(rgb)*SS(v2) -SS(I0*v2)];
% CRB = [zeros(3) -m*SS(v1)-m*SS(SS(v2)*rgb'); -m*SS(v1)-m*SS(SS(v2)*rgb')  m*SS(SS(v1)*rgb')-SS(I0*v2)]
% CRB1=[zeros(3) -SS(M11*v1 + M12*v2); -SS(M11*v1 + M12*v2) -SS(M21*v1 + M22*v2)]
...
h_xi_u = [zeros(6,1); inv(M)*ui] + [zeros(6,1); inv(M)*dw];
xidot = g_xi + h_xi_u; 
